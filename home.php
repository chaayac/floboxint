<?php
	...
	if (!isset($_SESSION['login_type']) || isset($_POST['logout'])) {
		...
		# Check if token is in db already
		...
		# If it's not in the session, we need to get a permanent one and store it in the session for future use.
		if ($row[0] == "f") {
			...
			# Place tokens into variable to concatenate into string
			...
			# Alternatives?: randomise password; create a different table for twitter users
			if ($row[0] == "f") {
			...
			$_SESSION['oauth_token'] = $perm_oauth_token; # so that we can keep the session going with the current user
